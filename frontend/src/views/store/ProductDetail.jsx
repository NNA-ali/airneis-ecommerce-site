...
function ProductDetail() {
    ...
    const [isHovered, setIsHovered] = useState(false); // État local pour suivre si l'utilisateur survole l'image principale

    const handleImageHover = () => {
        setIsHovered(true); // Définir l'état comme survolé lorsque l'utilisateur survole l'image principale
    };

    const handleImageLeave = () => {
        setIsHovered(false); // Définir l'état comme non survolé lorsque l'utilisateur quitte l'image principale
    };
                                                ...
                                                <div className="p-3" key={index}>
                                                    <img
                                                       ...
                                                       style={{
                                                            ...
                                                            objectFit: "cover",
                                                            borderRadius: "10px",
                                                            cursor: "pointer" // Ajoutez un style de curseur pour indiquer que l'image est cliquable
                                                        }}
                                                        alt={`Gallery image ${index + 1}`}
                                                        className={`thumbnail ${isHovered ? 'scroll' : ''}`}
                                                        onClick={() => handleThumbnailClick(g.image)}
                                                        />  
                                                </div>
                                            ))}
                                                        ...
                                                        <div className='d-flex'>
                                                            {size?.map((s, index) => (
                                                                <div key={index} className='me-2'>
                                                                    <input type="hidden" className='size_name' value={`${s.length} x ${s.width}`} />
                                                                    <button onClick={(e) => handleSizeButtonClick(e,size)} className='btn btn-secondary size_button'>{`${s.length} x ${s.width}`}</button>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    </div>
                                                        ...
                                                        <div className='d-flex'>
                                                            {color?.map((c, index) => (
                                                                <div key={index}>
                                                                    ...
                                                                    <button className='btn p-3 me-2 color_button' onClick={handleColorButtonClick} style={{ backgroundColor: `${c.color_code}`, border: `2px solid ${c.name.toLowerCase() === "white" ? "black" : "transparent"}` }}></button>
                                                                </div>
                                                            ))}
                                                        </div>
                                                        <hr />
